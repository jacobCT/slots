...
## 6. Include scaling factor for unregulated harvest
...
## CT VAS keep/release as coded by legal minimum and adjusted for unregulated harvest
## release length distribution trimmed at 15-40 cm
## harvest length dist. trimmed 34-59 cm for numerical consistancy
## analysis is now all based on 2012-2015
...
##assum
##QUESTIONS
## 1. Should analysis be based on 2013-2015 or include last full selex block,
## 2012-2015
## 2. What do we want to use for length distribution?
##    a. MRIP data only?
##    b. MRIP + CTVAS, NYHB, ALS ?

## analysis by weight,

## current catch- this is the current catch from the time serise used for the
## harvest slot reductions are entered.
## IF SEASONAL CLOSURES are being evaluated, these annual harvest figures should
## be adjusted prior to this script
...
hr_table$s.har <- round(mean.catch$har * (hr_table$har / sum(hr_table$har)))
hr_table$s.dis <- round(mean.catch$dis * (hr_table$dis / sum(hr_table$dis)))
hr_table$s.catch <- hr_table$s.har + hr_table$s.dis ##har + discards= catch
hr_table$P.h <- hr_table$s.har / hr_table$s.catch
hr_table$P.l <- hr_table$s.catch / sum(hr_table$s.catch)
...
hr_table$h.est <- sum(hr_table$s.catch) * hr_table$P.l * hr_table$wt * 1 #P.h set to 1
...
candidate.slots.B0 <- harvest.redux[harvest.redux$pr.redux > pr.min &
                             harvest.redux$pr.redux < pr.max, ]
candidate.slots.increase <- harvest.redux[harvest.redux$pr.redux < 0, ]
dim(candidate.slots.increase)
range(harvest.redux$pr.redux)
sl.bag[sl.bag$pr.redux.B4 > pr.min &
                             sl.bag$pr.redux.B4 < pr.max, ]
candidate.slots.B4 <- candidate.slots.B4[c(-75, -76, -77)]
colnames(candidate.slots.B4)[74] <- "pr.redux"
...
p <- ggplot(data = harvest.redux,
    ...
    scale_x_continuous(breaks = round(seq(min(harvest.redux$slmin),
                                          max(harvest.redux$slmax), by = 2), 1),
                       sec.axis = dup_axis()) +
    scale_fill_gradientn(limits = c(-1.7, 1),
                         colours = rainbow(7)) +
