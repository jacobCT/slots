...
## IF SEASONAL CLOSURES are being evaluated, these annul harvest figures should
...
rLen$length_cm[rLen$length_cm == 53] <- 44 #pooling all fish < 15 cm
...
hr_table[, paste("s.catch", sep = "")] <- ##har + discards= catch
    (hr_table$s.har + hr_table$s.dis)
hr_table[, paste("P.h", sep = "")] <-
    (hr_table$s.har / hr_table$s.catch)
hr_table[, paste("P.l", sep = "")] <- hr_table$s.catch / sum(hr_table$s.catch)
...
hr_table[, paste("h.est", sep ="")] <-
    sum(hr_table$s.catch) * hr_table$P.l * hr_table$wt * 1 #P.h set to 1
hr_table$P.h
