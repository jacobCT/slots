...
candidate.slots.B4 <- sl.bag[sl.bag$pr.redux.B4 > 0.45 &
                             sl.bag$pr.redux.B4 < .50, ]
candidate.slots.B3 <- sl.bag[sl.bag$pr.redux.B3 > 0.45 &
                             sl.bag$pr.redux.B3 < .50, ]
candidate.slots.B2 <- sl.bag[sl.bag$pr.redux.B2 > 0.45 &
                             sl.bag$pr.redux.B2 < .50, ]
candidate.slots.B1 <- sl.bag[sl.bag$pr.redux.B1 > 0.45 &
                             sl.bag$pr.redux.B1 < .50, ]
...
lens <- seq(min(unaged$length), max(unaged$length), 1)
...
for (r in 1:nrow(yrs.tmp)){
    ...
    unaged.y <- unaged[unaged$year %in% y2, ]
    ...
    tmp.len.mod <-  colSums(prop.mlr * as.numeric(tmp.unage[, 2]))
