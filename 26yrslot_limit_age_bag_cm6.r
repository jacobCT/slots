...
for(b in 1:4){ ##loop over each bag limit option
    ...
    sl.tmp <- harvest.redux[harvest.redux[paste("pr.redux.B", b,  sep ="")] >
              pr.min & harvest.redux[paste("pr.redux.B", b,  sep ="")] <
              pr.max, ]
    col.tmp <- harvest.redux[paste("pr.redux.B", b,  sep ="")][harvest.redux
               [paste("pr.redux.B", b,  sep ="")] > pr.min &
                harvest.redux[paste("pr.redux.B", b,  sep ="")] < pr.max, ]
    harvestN <- t(sl.tmp[, 6:53])
    harvestN <- (rbind(harvestN, col.tmp))
    harvestN[is.na(harvestN)] <- 0
    rownames(harvestN)[49] <- paste("pr.redux.B", b,  sep ="")
    selex <- NULL
    for(c in 1:ncol(harvestN)){
        ##calculate harvest-at-age
        tmp.age.har <- colSums(prop.mlr * harvestN[c(3:c(nrow(harvestN)- 1)), c])
